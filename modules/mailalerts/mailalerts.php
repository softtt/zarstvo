...
class MailAlerts extends Module
{
	...
	public function hookActionValidateOrder($params)
	{
		...
		$template_vars = array(
			...
			'{total_shipping}' => $order->total_shipping > 0 ? Tools::displayPrice($order->total_shipping, $currency) : 'По тарифам перевозчика',
		...
		foreach ($merchant_mails as $merchant_mail)
		{
			...
			if ($dir_mail)
				Mail::Send(
					$mail_id_lang,
					'new_order',
					sprintf(Mail::l('Новый заказ: №%d', $mail_id_lang), $order->id),
					$template_vars,
					$merchant_mail,
					null,
