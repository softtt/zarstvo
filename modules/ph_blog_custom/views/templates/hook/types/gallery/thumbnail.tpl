...
{if isset($post.banner) && Configuration::get('PH_BLOG_DISPLAY_THUMBNAIL')}
	<div class="post-thumbnail" itemscope itemtype="http://schema.org/ImageObject">
		<a {if isset($post.gallery) && sizeof($post.gallery)}href="{$gallery_dir|escape:'html':'UTF-8'}{$post.gallery.0.image|escape:'html':'UTF-8'}.jpg" data-fancybox-group="post-gallery-slideshow-{$post.id_simpleblog_post|intval}" class="post-gallery-link"{else}href="{$post.url|escape:'html':'UTF-8'}"{/if} title="{l s='Permalink link to' mod='ph_blog_custom'} {$post.title|escape:'html':'UTF-8'}" itemprop="contentUrl">
