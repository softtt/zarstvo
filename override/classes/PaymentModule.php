...
abstract class PaymentModule extends PaymentModuleCore
{
        ...
        $secure_key = false, Shop $shop = null)
    {
        ...
        $this->context->customer = Context::getContext()->customer;

        // The tax cart is loaded before the customer so re-cache the tax calculation method
        $this->context->cart->setTaxCalculationMethod();
        $this->context->cart->checkZeroQuantities();
        ...
        if (Validate::isLoadedObject($this->context->cart) && $this->context->cart->OrderExists() == false)
        {
            ...
            foreach ($package_list as $id_address => $packageByAddress)
                foreach ($packageByAddress as $id_package => $package)
                {
                    ...
                    $order->total_discounts = $order->total_discounts_tax_excl;

                    if ($is_free_shipping) {
                        $order->total_discounts_tax_excl += $order->total_shipping_tax_excl;
                        $order->total_discounts_tax_incl += $order->total_shipping_tax_excl;
                        $order->total_discounts += $order->total_shipping_tax_excl;
                    }
                    ...
                    // Total order price is calculating here!!!
                    $order->total_paid_tax_excl = $order->total_products - $order->total_discounts + $order->total_shipping;
            ...
            foreach ($order_detail_list as $key => $order_detail)
            {
                ...
                if (!$order_creation_failed && isset($order->id))
                {
                    ...
                    if ($id_order_state != Configuration::get('PS_OS_ERROR') && $id_order_state != Configuration::get('PS_OS_CANCELED') && $this->context->customer->id)
                    {
                        ...
                        '{total_shipping}' => $order->total_shipping > 0 ? Tools::displayPrice($order->total_shipping, $this->context->currency, false) : "По тарифам перевозчика",
...
//                        '{total_tax_paid}' => Tools::displayPrice(($order->total_products_wt - $order->total_products) + ($order->total_shipping_tax_incl - $order->total_shipping_tax_excl), $this->context->currency, false)
            ...
            $export = new Export();
            $export->exportOrder($order->id);

            // Create virtual discount card for user if not created and overall buys sum over 2000
            // TODO: Add minimun accumulated sum assignment to admin panel
            // TODO: Send email to customer with newly create discount card
            if (!$this->context->customer->getDiscountCard() && $this->context->customer->getStats()['total_orders'] >= 2000)
            {
                // Assign discount card to customer if it is stored in DB
                if (Validate::isEmail($this->context->customer->email)) {
                        $email = $this->context->customer->email;
                        ...
                        if ($discountCards) {
                            ...
                            if (!$d->id_customer)
                            {
                                $d->id_customer = $this->context->customer->id;
                            ...
                            $card->id_customer = $this->context->customer->id;
                            $card->email = $this->context->customer->email;
                            $card->is_virtual = true;
                            $card->virtual_discount_code = DiscountCard::generatePersonalDiscountCode();
                            if ($card->save())
                            {
                                if (Validate::isEmail($this->context->customer->email))
                                    Mail::Send(
                                        (int)$order->id_lang,
                                        'discount_card_created',
                                        Mail::l('Personal discount created', (int)$order->id_lang),
                                        array(
                                            '{firstname}' => $this->context->customer->firstname,
                                            '{discount_code}' => $card->getCardCode(),
                                            '{discount_percent}' => (int)$card->getDiscountPercent(),
                                        ),
                                        $this->context->customer->email,
                                        $this->context->customer->firstname.' '.$this->context->customer->lastname
                                    );
