...
<div class="box">
    ...
    {else}
        ...
        <form action="{$link->getPageLink('identity', true)|escape:'html':'UTF-8'}" method="post" class="std" enctype="multipart/form-data">
            <fieldset>
                ...
                <div class="form-group">
                    ...
                    <div class="row avatarUpload" data-provides="fileinput">
                        <img src="{$avatar['path']}" title="Фотография" width="{$avatar['width']}" height="{$avatar['height']}" data-url="index.php" id="avatarImg"/>
                        {if $avatar['canRemove']}
                        <i class="icon-remove" id="removeAvatar"></i>
                        {/if}
                        <input type="hidden" name="removeAvatar" value="0">
                    </div>
                    <input class="form-control col-sm-6" type="file" name="avatar" id="avatar"/>
                </div>
                {if $newsletter}
...
<script type="text/javascript">
    $('#removeAvatar').on('click', function () {
        $(this).hide();
        var removeAvaFlag = $("input[name='removeAvatar']");
        var imgObj = $('#avatarImg');
        removeAvaFlag.val((removeAvaFlag.val() == '0' ? '1' : '0'));
        imgObj.attr('src', "{$defaultAvatarPath}");
    })
</script>