...
<p class="info-account discount-info">
    {if isset($discount_code)}
        {l s='Your personal discount code: '} <b>{$discount_code}</b>.
    {/if}
    {if isset($discount_percent)}
        {l s='Size of accumulated discount percent:'} <b>{$discount_percent|number_format}%</b>.
    {/if}
</p>
...
<div class="row addresses-lists">
    <div class="col-xs-12 col-sm-6 col-lg-4">
        <ul class="myaccount-link-list">
            ...
            <li><a href="{$link->getPageLink('security', true)|escape:'html':'UTF-8'}" title="{l s='Security'}"><i class="icon-lock"></i><span>{l s='Security'}</span></a></li>
        </ul>
	</div>
