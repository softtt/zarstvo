...
$(document).ready(function(){
    $('#cart_summary, #layer_cart').on('click', '.cart_quantity_up', function(e){
        e.preventDefault();
        upQuantity($(this).attr('id').replace('cart_quantity_up_', ''));
    });
    $('#cart_summary, #layer_cart').on('click', '.cart_quantity_down', function(e){
        e.preventDefault();
        downQuantity($(this).attr('id').replace('cart_quantity_down_', ''));
    });
    $('#cart_summary, #layer_cart').on('click', '.cart_quantity_delete', function(e){
        e.preventDefault();
    ...
    $(document).on('click', 'input[name=id_payment]', function(){
        updatePrepaymentBonusShow();
    });

    $(document).on('click, change', 'input[name=bonus]', function() {
        $('.bonus_name').removeClass('selected');
        var type = $(this).attr('id').substring(0, $(this).attr('id').indexOf( "_id" ));
        $('.bonus_name[data-type=' + type + ']').addClass('selected');
    ...
    $('input[name=bonus]').each(function() {
        var type = $(this).attr('id').substring(0, $(this).attr('id').indexOf( "_id" ));
        $(this).val($('.bonus_name[data-type=' + type + ']').data('rule-id'));
    });

    $(document).on('click', '.bonus_name', function() {
        $('.bonus_name').removeClass('selected');
        var type = $(this).data('type');
        $('#select_bonus #' + type + '_id').prop("checked", true).trigger("click");
        $(this).addClass('selected');

        updateCartSummaryTotalPrice();
    });

    UpdateSummatyTypeWatch();
    updatePrepaymentBonusShow();

    $('.layer_cart_quantity_input').typeWatch({
        highlight: true, wait: 400, captureLength: 0, callback: function(val){
            ...
            else
            {
                ...
                if (parseInt(jsonData.summary.products.length) == 0)
                {
                    if ((typeof(orderProcess) == 'undefined' || orderProcess !== 'order-opc')
                        && !(orderProcess === 'product' || orderProcess === 'product-list')
                    ) {
                ...
                // if ($('body').attr('id') == 'order-opc' && jsonData.shopping_cart_tbody) {
                //     $('#cart_summary tbody').replaceWith(jsonData.shopping_cart_tbody);
                //     UpdateSummatyTypeWatch();
                // }
                updateCartSummary(jsonData.summary);
                if (window.ajaxCart != undefined)
...
function updateCartSummary(json)
{
    ...
    for (i in product_list)
    {
        ...
        if (reduction && typeof(initial_price) !== 'undefined')
        {
            ...
            else
            {
                ...
                price_reduction = floorf((product_list[i].price_without_quantity_discount - display_price)/product_list[i].price_without_quantity_discount * -100, 0);
                reduction_symbol = '%';
            }

            if (initial_price !== '' && product_list[i].price_without_quantity_discount > product_list[i].price)
            {
                initial_price_text = '<li class="old-price">' + initial_price + '</li><li class="price-percent-reduction small">&nbsp;'+price_reduction+reduction_symbol+'&nbsp;</li>';
        ...
        $('#product_price_' + key_for_blockcart).html(initial_price_text + '<li class="' + current_price_class + '">' + current_price + '</li>');
        if (typeof(product_list[i].customizationQuantityTotal) !== 'undefined' && product_list[i].customizationQuantityTotal > 0) {
            ...
            $('#layer_cart .layer_cart_product_price_' + key_for_blockcart).html(formatCurrency(product_customization_total, currencyFormat, currencySign, currencyBlank));
        } else {
            $('#total_product_price_' + key_for_blockcart).html(formatCurrency(product_total, currencyFormat, currencySign, currencyBlank));
            $('#layer_cart .layer_cart_product_price_' + key_for_blockcart).html(formatCurrency(product_total, currencyFormat, currencySign, currencyBlank));
        }
    ...
    window.total_price = json.total_price;
    window.total_products = json.total_products;
    window.discounts = json.discounts;
    console.log(json.total_shipping_tax_exc);
    window.total_shipping_cost = json.total_shipping_tax_exc;

    if (!json.discounts.length)
    {
        $('.cart_total_voucher').hide();
    }
    else
    {
        if (json.discounts.length > 1) {
            $('.cart_total_voucher').hide();
            $('.cart_order_bonus').show();
            var html = '';
            var len = json.discounts.length;
            var selected_id = $('input[name=bonus]:checked').val();
            var selected_type = $('input[name=bonus]:checked').attr('id');

            json.discounts.forEach(function(e, i) {
                var type = e.free_shipping == 1 ? 'free_shipping' : 'discount';
                html += '<p class="bonus_name" id="bonus_name_' + e.id_cart_rule + '"'
                        + ' data-type="' + (e.free_shipping == 1 ? 'free_shipping' : 'discount')
                        + '">'
                        + e.name;

                if (e.free_shipping == 0) {
                    html += ' <i>-' + formatCurrency(e.value_real, currencyFormat, currencySign, currencyBlank) + '</i></p>';
                    $('.cart_total_voucher .discount_title').html(e.name);
                    $('#select_bonus #discount_id').val(e.id_cart_rule);
                    $('#select_bonus label[for=discount_id]').html(e.name + ' (' + formatCurrency(e.value_real, currencyFormat, currencySign, currencyBlank) + ')');
                } else {
                    $('#select_bonus #free_shipping_id').val(e.id_cart_rule);
                }
            ...
            $('.cart_order_bonus .bonuses_list').html(html);
            if (selected_id)
                $('#bonus_name_' + selected_id).addClass('selected');
            else if (selected_type)
                $('.bonus_name[data-type=' + selected_type.substring(0, selected_type.indexOf( "_id" )) + ']').addClass('selected');
            $('#select_bonus #discount').attr('value', '');
            ...
            $('.cart_order_bonus').hide();
            $('.cart_total_voucher').show();
            $('#select_bonus').hide();
            if (json.discounts[0]) {
                $('.cart_total_voucher .discount_title').html(json.discounts[0].name);
                if (json.discounts[0].free_shipping == 0)
                    $('#select_bonus #discount_id').val(json.discounts[0].id_cart_rule);
    ...
    $('#total_shipping_cost').html(formatCurrency(json.total_shipping_tax_exc, currencyFormat, currencySign, currencyBlank));

    if (typeof(showTotalShippingNotes) !== 'undefined') {
        showTotalShippingNotes(json.total_shipping_tax_exc);
    }
}

function updateCartSummaryTotalPrice() {
    if ($('.bonus_name.selected').data('type') == 'free_shipping')
        $('#total_price').html(formatCurrency(window.total_products, currencyFormat, currencySign, currencyBlank));
    else
        $('#total_price').html(formatCurrency(window.total_price, currencyFormat, currencySign, currencyBlank));
}
...
function UpdateSummatyTypeWatch() {
...
function updatePrepaymentBonusShow() {
    if ($('#payment_method_prepayment').is(':checked') && window.discounts.length > 1) {
        $('.cart_order_bonus').show();
        $('.cart_total_voucher').hide();
        $('#select_bonus').show();
        $('#select_bonus label[for=discount_id]').html($('.cart_order_bonus .bonus_name[data-type=discount]').html());
    } else {
        $('.cart_order_bonus').hide();
        $('#select_bonus').hide();
        if (window.discounts && window.discounts.length > 1)
            $('.cart_total_voucher').show();
    }
}
