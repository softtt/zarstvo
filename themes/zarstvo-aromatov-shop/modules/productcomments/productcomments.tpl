    ...
    <div id="product_comments_block_tab">
        {if $comments}
            {foreach from=$comments item=comment}
                ...
                {if $comment.total_useful < ( (int)$comment.total_advice/2 ) }
                <div class="comment row transparent" itemprop="review" itemscope itemtype="http://schema.org/Review">
                {else}
                <div class="comment row" itemprop="review" itemscope itemtype="http://schema.org/Review">
                {/if}
                    <div class="comment_details col-sm-12 test">
                        <div class="comment_header">
                            <span itemprop="author" class="author_name">{$comment.customer_name|escape:'html':'UTF-8'}</span>
                            <meta itemprop="datePublished" content="{$comment.date_add|escape:'html':'UTF-8'|substr:0:10}" />
                            <span class="date_published">{dateFormat date=$comment.date_add|escape:'html':'UTF-8' full=0}</span>
                        ...
                        <p itemprop="reviewBody" class="clearfix review_body">{$comment.content|escape:'html':'UTF-8'|nl2br}</p>
                        <ul>
                            {if $is_logged}                
        ...
        <form id="id_new_comment_form" action="#">
            ...
            <table class="product clearfix">
                {if isset($product) && $product}
                    <tr>
                        <td rowspan="3" class="product_image">
                    ...
                    <tr>
                        <td class="new_comment_form_content">
                            <label for="comment_title">
                                {l s='Title:' mod='productcomments'} <sup>*</sup>
                            </label>
                            <input id="comment_title" name="title" type="text" value="" class="input_required"/>
                            <label for="content">
                                {l s='Comment:' mod='productcomments'} <sup>*</sup>
                            </label>
                            <textarea id="content" name="content" class="input_required"></textarea>
                            {if $allow_guests == true && !$is_logged}
                                <label>
                                    {l s='Your name:' mod='productcomments'} <sup>*</sup>
                                </label>
                                <input id="commentCustomerName" name="customer_name" type="text" value="" class="input_required"/>
                            {/if}
