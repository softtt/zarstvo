...
{if isset($cms) && !isset($cms_category)}
    ...
    {/if}
    <h1 class="page-heading">{$cms->meta_title|escape:'html':'UTF-8'}</h1>
    <div class="cms-page-content rte{if $content_only} content_only{/if}">
        {$cms->content}
    </div>
{elseif isset($cms_category)}
    <div class="block-cms">
        <h1 class="page-heading"><a href="{if $cms_category->id eq 1}{$base_dir}{else}{$link->getCMSCategoryLink($cms_category->id, $cms_category->link_rewrite)}{/if}">{$cms_category->name|escape:'html':'UTF-8'}</a></h1>
