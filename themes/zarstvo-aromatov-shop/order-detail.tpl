...
<div class="box clearfix">
	<form id="submitReorder" action="{if isset($opc) && $opc}{$link->getPageLink('order-opc', true)}{else}{$link->getPageLink('order', true)}{/if}" method="post" class="submit">
			...
			<a href="#" onclick="$(this).closest('form').submit(); return false;" class="link-button pull-right"><span><i class="icon-refresh"></i>{l s='Reorder'}</span></a>
			<p>
...
<div class="col-xs-12 col-sm-6 no-padding"{if $order->isVirtual()} style="display:none;"{/if}>
	...
	<table class="table table-bordered">
		...
		<tfoot>
			...
			<tr class="item">
				...
				<td colspan="{if $order->hasProductReturned()}5{else}4{/if}">
					<span class="price-discount">{displayWtPriceWithCurrency price=$order->total_shipping_tax_excl currency=$currency convert=1}</span>
				</td>
			</tr>
			{/if}
			...
			{if !isset($product.deleted)}
				...
				{if isset($product.customizedDatas)}
					<tr class="item">
						...
						<td>
							<label for="cb_{$product.id_order_detail|intval}"><a href="{$product.link|escape:'html':'UTF-8'}">{$product.product_name|escape:'html':'UTF-8'}</a></label>
						</td>
						<td>
						...
						<td class="product-link">
							<label for="cb_{$product.id_order_detail|intval}">
								{if $product.download_hash && $invoice && $product.display_filename != '' && $product.product_quantity_refunded == 0 && $product.product_quantity_return == 0}
									...
									{/if}
								{else}
									<a href="{$product.link|escape:'html':'UTF-8'}">{$product.product_name|escape:'html':'UTF-8'}</a>
								{/if}
							</label>
						</td>
	...
	<table class="table table-bordered footab">
		<thead>
			<tr>
				...
				{if isset($followup)}
					<th class="item" data-sort-ignore="true">{l s='Tracking link'}</th>
				{/if}
			</tr>
		</thead>
		<tbody>
...
{if !$is_guest}
	...
	<form action="{$link->getPageLink('order-detail', true)|escape:'html':'UTF-8'}" method="post" class="std" id="sendOrderMessage">
		...
		<div class="submit">
			...
			<button type="submit" name="submitMessage" class="button btn btn-default button-medium"><span>{l s='Send'}</span></button>
		</div>
	</form>
{/if}
{/if}
