	...
	public function displayListContent()
	{
		...
		foreach ($this->_list as $index => $tr)
		{
			...
			foreach ($this->fields_list as $key => $params)
			{
				...
				}elseif (isset($params['statusSelector'])) {

					if (method_exists($this->context->controller, 'displayStatusSelector')) {
						$status_selector_tpl = $this->createTemplate('list_status_selector.tpl');

						$this->_list[$index][$key] = $this->context->controller->displayStatusSelector(
							$status_selector_tpl,
							$this->token,
							$id,
							$tr[$key],
				...
				} elseif (isset($params['specialInput'])) {
					if (method_exists($this->context->controller, 'displaySpecialInput')) {
						$tpl = $this->createTemplate('list_special_input.tpl');

						$this->_list[$index][$key] = $this->context->controller->displaySpecialInput(
							$tpl,
							$this->token,
							$id
						);
