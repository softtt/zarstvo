...
class IdentityControllerCore extends FrontController
{
	...
	public function postProcess()
	{
		...
		if (Tools::isSubmit('submitIdentity'))
		{
			...
			$removeAvatar = Tools::getValue('removeAvatar', false) == '1' ? true : false;

			if ($removeAvatar) {
				$this->customer->avatar = false;
			} else {
				$image_uploader = new HelperImageUploader('avatar');
				...
				if (!count($this->errors) && isset($file) && empty($file['error'])) {
					$error = 0;
					$newPath = $this->customer->getAvatarPathCreation();
	...
	public function initContent()
	{
		...
		$this->context->smarty->assign(array(
				...
				'avatar' => array(
					'path'=>$this->customer->getUserAvatarPathFrontend(),
					'width'=>Customer::$avatarSize['full'][0],
					'height'=>Customer::$avatarSize['full'][1],
					'canRemove'=>$this->customer->avatar
				),
				'defaultAvatarPath'=>$this->customer->getDefaultAvatarPath()
